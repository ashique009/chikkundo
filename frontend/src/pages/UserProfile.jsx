  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass-panel p-6 rounded-3xl border border-[#F4C0D1] dark:border-brand-purple/10 bg-white dark:bg-brand-dark/20 text-center flex flex-col items-center transition-colors duration-200">
          {avatar ? (
            <img
              src={avatar}
          ...
          <div className="mt-6 w-full border-t border-[#F4C0D1] dark:border-slate-900 pt-6 space-y-4 text-left">
            ...
            {userProfile.date_of_birth && (
              <div className="flex items-center gap-3 text-[#5F5E5A] dark:text-slate-400">
                <Calendar className="w-4 h-4 text-[#5F5E5A] dark:text-slate-500" />
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Bio Box */}
          <div className="glass-panel p-6 rounded-3xl border border-[#F4C0D1] dark:border-brand-purple/10 bg-white dark:bg-brand-dark/10">
            <h4 className="text-xs font-extrabold text-[#5F5E5A] dark:text-slate-500 uppercase tracking-widest mb-3">About Me</h4>
            <p className="text-[#2C2C2A] dark:text-slate-300 text-sm leading-relaxed whitespace-pre-line">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="glass-panel p-5 rounded-2xl border border-[#F4C0D1] dark:border-brand-purple/10 bg-white dark:bg-brand-dark/10 flex items-center gap-4">
              <div className="p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-400">
                <Heart className="w-4.5 h-4.5" />
              </div>
              <div>
                <div className="text-[10px] font-extrabold text-[#5F5E5A] dark:text-slate-500 uppercase tracking-widest">Looking For</div>
                <div className="text-sm font-bold text-[#2C2C2A] dark:text-slate-200 mt-0.5">
            ...
            <div className="glass-panel p-5 rounded-2xl border border-[#F4C0D1] dark:border-brand-purple/10 bg-white dark:bg-brand-dark/10 flex items-center gap-4">
              ...
              <div>
                <div className="text-[10px] font-extrabold text-[#5F5E5A] dark:text-slate-500 uppercase tracking-widest">Gender</div>
                <div className="text-sm font-bold text-[#2C2C2A] dark:text-slate-200 mt-0.5">
          ...
          {/* Interests Tags */}
          <div className="glass-panel p-6 rounded-3xl border border-[#F4C0D1] dark:border-brand-purple/10 bg-white dark:bg-brand-dark/10">
            <h4 className="text-xs font-extrabold text-[#5F5E5A] dark:text-slate-500 uppercase tracking-widest mb-4">My Interests</h4>
            {userProfile.interests && userProfile.interests.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {userProfile.interests.map((interest) => (
                  <span
                    key={interest.id}
                    className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-[#D4537E]/10 dark:bg-brand-purple/10 border border-[#D4537E]/20 dark:border-brand-purple/20 text-[#D4537E] dark:text-brand-purple-light"
                  >
                    {interest.name}
                  </span>
                ))}
              ...
              <p className="text-[#5F5E5A] dark:text-slate-500 text-xs">No interest tags selected.</p>
            )}
          </div>
        </div>
      </div>
